...
public class BoardBuilder implements IBoardBuilder {
    ...
    private int findPositionInValuesPool(Map<Integer, Integer> zeroPositionMap){
        int boardSize = board.getBoardSize();
        int boardSide = (int) Math.sqrt(boardSize);

        List<Integer> keys = new ArrayList<>();
        for (Integer integer : zeroPositionMap.keySet()) {
            keys.add(integer);
        }

        int keyInZeroPositionMap = keys.get(0);
        int valueInZeroPositionMap = zeroPositionMap.get(keyInZeroPositionMap);

        int positionKeyAux = keyInZeroPositionMap - 1;
        int positionAccumulator = positionKeyAux * boardSide;
        positionAccumulator += valueInZeroPositionMap;
        return positionAccumulator;
    }

    // Ejecutar 1ero
    @Override
    public void putIntoValuesPoolWithSpecificZeroPosition(Map<Integer, Integer> zeroPositionMap){
        int positionAccumulator = findPositionInValuesPool(zeroPositionMap);
        for (int i = 1; i <= board.getBoardSize(); i++) {

            if ((positionAccumulator == board.getBoardSize()) && (i == positionAccumulator)){
                board.getPoolValues().add(0);
                break;
            }
            if (i == positionAccumulator){
                board.getPoolValues().add(0);
                i--;
                positionAccumulator = positionAccumulator * -1;
                continue;
            }
            board.getPoolValues().add(i);
        }
